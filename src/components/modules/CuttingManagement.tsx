...
import Navbar from "@/components/ui/Navbar"; // Add the Navbar import here
...
const CuttingManagement = () => {
      ...
      accessor: (item) =>
        editingId === item.id ? (
          <select
            ...
            className="w-full rounded-md border border-gray-600 bg-white text-black px-2 py-1 text-sm"
            autoFocus
          >
        ...
        editingId === item.id ? (
          <div className="flex space-x-2">
            <button
              ...
              disabled={saving}
              className="inline-flex items-center rounded-md bg-gray-900 px-4 py-2 text-xs text-white hover:bg-gray-800 transition-all"
            >
              {saving ? (
          ...
          <button
            ...
            className="inline-flex items-center rounded-md bg-gray-600 px-4 py-2 text-xs text-white hover:bg-gray-500 transition-all"
          >
            Edit
          </button>
        ),
  ...
  return (
    <div className="flex">
      <div className="fixed top-0 left-0 w-64 h-full bg-gray-800 text-white"> {/* Sidebar/ Navbar */}
        <Navbar /> {/* Add Navbar here */}
      </div>

      {/* Content Section */}
      <div className="ml-64 w-full container mx-auto max-w-6xl px-6 py-8 bg-white text-black rounded-lg shadow-xl">
        ...
        <div className="mb-6">
          <form onSubmit={handleCustomerIdSubmit} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
            <div className="w-full sm:w-2/3">
              <label htmlFor="customerId" className="mb-2 block text-sm font-medium text-black">
                Enter Customer ID
              </label>
              <Input
                id="customerId"
                ...
                className="w-full text-sm p-3 rounded-md border border-gray-600 bg-white text-black focus:outline-none focus:ring-2 focus:ring-gray-700"
              />
            </div>
            <button
              type="submit"
              className="mt-4 sm:mt-0 sm:self-end rounded-md bg-gray-700 px-6 py-2 text-sm font-medium text-white hover:bg-gray-600 focus:ring-2 focus:ring-gray-600 transition-all"
            >
              Load Data
            </button>
          </form>
        ...
        <form ref={formRef}>
          <DataTable
            ...
            isLoading={loading}
            tableClassName="table-auto w-full text-black bg-black"
            cellClassName="border-t border-gray-200"
            headerClassName="border-b border-gray-300 bg-gray-100 text-black"
          />
        </form>

        <div className="mt-6 rounded-md bg-gray-100 p-4 text-sm text-black">
